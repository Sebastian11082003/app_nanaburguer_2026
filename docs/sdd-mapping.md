...
| 11-quality | parcial | Jest en backend (roles, órdenes, caja, cierre de turno, reportes con rango, alta de estaciones). Reportes/caja tienen UI sobre el API existente. No hay `testing-strategy.md`. Huecos de HU vs código en [mvp-production-readiness.md](mvp-production-readiness.md). |
| 12-ux-ui | parcial | Login único en `/restaurant/login` (local-login redirige ahí): plataforma hasta identificar slug a mano (opcional); el correo del restaurante entra como ADMIN si no hay User y abre el POS, sin segunda pantalla de rol. Acceso del local por correo, no slug+email. Chrome de tenant después. Plataforma puede inhabilitar el tenant. Nav por rol. Piso: ocupada a $0 muestra “Sin cuenta”; salir de ticket vacío libera solo mesa. Llevar/domicilio se retoman; un CREATED vacío se descarta a mano. Pedidos activos es cola DELIVERY. Caja cobra domicilio en camino/entregado. Ventas y cocina nombran mesa/Llevar/Domicilio. Llevar abierto dice En cocina/Listo. Despachar saca el ticket del KDS (`OUT_FOR_DELIVERY`). Rider no ve Llevar en el piso ni Pickup al crear. KDS con líneas, poll y CANCELADO. Recibo sin líneas anuladas. Caja/admin en `restaurant-navigation.ts`. Alta de mesero/caja/cocina/domicilio al crear tenant o desde admin. |
