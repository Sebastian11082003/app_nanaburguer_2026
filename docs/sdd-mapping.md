...
| 11-quality | parcial | 55 tests Jest en backend. Reportes/caja ahora tienen UI sobre el API existente. No hay `testing-strategy.md`. |
| 12-ux-ui | parcial | Login único + chrome de tenant (logotipo/nombre/slug) en personal, POS, admin y hubs. Nav por rol. Piso: ocupada a $0 muestra “Sin cuenta”; salir de ticket vacío libera la mesa. Mostrador lista Llevar abiertos. Domicilio activo tiene alta. Caja/admin en `restaurant-navigation.ts`. |
