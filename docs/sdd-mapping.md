...
| 12-ux-ui | parcial | Login único + chrome de tenant (logotipo/nombre/slug) en personal, POS, admin y hubs. Nav por rol. Piso: ocupada a $0 muestra “Sin cuenta”; salir de ticket vacío libera solo mesa. Llevar/domicilio se retoman; un CREATED vacío se descarta a mano. Pedidos activos es cola DELIVERY (sin cobrados pendientes de despacho). Caja cobra desde Domicilios también en camino/entregado. Ventas nombra mesa/Llevar/Domicilio, filtra abiertas y marca En camino/Entregado. Caja/admin en `restaurant-navigation.ts`. |
