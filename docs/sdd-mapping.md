...
| 11-quality | parcial | Jest en backend (auth, órdenes, guards, caja, delivery, users, reportes, env). Sin E2E. Corte de piloto local: [mvp-production-readiness.md](mvp-production-readiness.md). |
| 12-ux-ui | parcial | Login único: plataforma hasta identificar slug a mano; el correo del restaurante entra como ADMIN si no hay User. Acceso del local por correo, no slug+email. Chrome de tenant después. Plataforma puede inhabilitar el tenant. Nav de plataforma: Dashboard + Restaurantes (sin 404). Nav por rol. Piso: ocupada a $0 muestra “Sin cuenta”; salir de ticket vacío libera solo mesa. Llevar/domicilio se retoman; un CREATED vacío se descarta a mano. Pedidos activos es cola DELIVERY. Caja cobra domicilio en camino/entregado. Ventas y cocina nombran mesa/Llevar/Domicilio. Llevar abierto dice En cocina/Listo. Despachar saca el ticket del KDS (`OUT_FOR_DELIVERY`). Rider no ve Llevar en el piso ni Pickup al crear. KDS con líneas y poll. Caja/admin en `restaurant-navigation.ts`. Alta de mesero/caja/cocina/domicilio al crear tenant o desde admin. |
| 13-operations | parcial | Runbook local + overlay HTTPS en [10-devops/local-setup.md](10-devops/local-setup.md). Piloto = Docker local; VPS fuera del recorte actual. [mvp-production-readiness.md](mvp-production-readiness.md). |
