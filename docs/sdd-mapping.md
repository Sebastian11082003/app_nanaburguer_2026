...
| 12-ux-ui | parcial | Login único + chrome de tenant (logotipo/nombre/slug) en personal, POS, admin y hubs. Nav por rol. Piso: ocupada a $0 muestra “Sin cuenta”; salir de ticket vacío libera solo mesa (no pickup/domicilio). Mostrador lista Llevar abiertos con nombre; domicilio retoma borradores. Pedidos activos es cola DELIVERY (sin cobrados pendientes de despacho). Caja cobra y no despacha el borrador CREATED. Ventas nombra mesa/Llevar/Domicilio y filtra abiertas. Caja/admin en `restaurant-navigation.ts`. |
